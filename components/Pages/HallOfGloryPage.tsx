    ...
    <div className="mb-20">
      <div className={`relative py-12 px-6 bg-gradient-to-br ${themeStyles.gradient} via-white dark:via-slate-900 to-transparent rounded-[3rem] border ${themeStyles.border} overflow-hidden shadow-2xl transition-all`}>
        ...
        <div className="flex flex-col items-center mb-12 relative z-10 text-center">
          ...
          <h2 className={`text-3xl md:text-5xl font-black ${themeStyles.text} mb-3 tracking-tight transition-colors`}>
            {title}
          </h2>
          <p className="text-base md:text-xl text-slate-600 dark:text-slate-400 max-w-2xl italic px-4 transition-colors">
            {description}
          </p>
        </div>
          ...
          {achievements.map((person) => (
            <div key={person.id} className="group relative">
              <div className={`h-full relative bg-white/70 dark:bg-slate-950/50 backdrop-blur-md p-6 rounded-[2rem] border ${themeStyles.border} text-center flex flex-col items-center transition-all duration-500 hover:border-emerald-500/40 hover:-translate-y-2 hover:shadow-2xl`}>
                ...
                <h3 className="text-lg font-black text-slate-900 dark:text-white mb-1 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                  {person.name}
                </h3>
                <span className={`${themeStyles.badge} text-white px-3 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest mb-3`}>
                  {tag}
                </span>
                <p className="text-slate-600 dark:text-slate-400 text-xs leading-relaxed mb-4 line-clamp-3 transition-colors">
                  {person.description}
                </p>
                <div className={`mt-auto pt-3 border-t border-slate-200 dark:border-slate-800/50 w-full text-sm italic ${themeStyles.text} font-black opacity-80`}>
                  {person.year}
                </div>
              </div>
            </div>
